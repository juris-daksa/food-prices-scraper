  ...
  try {
    const items = await page.evaluate(() => {
      ...
      productElements.forEach((product) => {
        ...
        const discountPriceElement = product.querySelector("span.special-price .price-wrapper .price");
        const finalPriceElement = product.querySelector("span[data-price-type='finalPrice'] .price");

        const title = titleElement ? titleElement.innerText.trim() : null;
        let retailPrice = finalPriceElement 
          ? parseFloat(finalPriceElement.innerText.replace(',', '.').replace('€', '').trim()) 
          : null;

        if (oldPriceElement) {
          retailPrice = parseFloat(oldPriceElement.innerText.replace(',', '.').replace('€', '').trim());
        }

        let discountPrice = discountPriceElement
          ? parseFloat(discountPriceElement.innerText.replace(',', '.').replace('€', '').trim())
          : null;
        ...
        const multiPackMatch = title.match(/(\d+(\.\d+)?)\s?g[xX](\d+)/i);
        const pieceMultiMatch = title.match(/(\d+)\s?x\s?(\d+)\s?gab\./i);

        if (pieceMultiMatch) {
          unitSize = parseInt(pieceMultiMatch[1]) * parseInt(pieceMultiMatch[2]);
          unit = "gab.";
        } else if (multiPackMatch) {
          unitSize = parseFloat(multiPackMatch[1]) * parseInt(multiPackMatch[3]);
          unit = "kg";
        } else if (weightMatches) {
          const weights = weightMatches.map(w => parseFloat(w.match(/\d+(\.\d+)?/)[0]));
          ...
          const volumes = volumeMatches.map(v => parseFloat(v.match(/\d+(\.\d+)?/)[0]));
        ...
        const unitPrice = unitSize && retailPrice
          ? parseFloat((retailPrice / unitSize).toFixed(2))
          : retailPrice;
        const discountUnitPrice = unitSize && discountPrice
